...
vi.mock("./llama-server-install.js", async (importOriginal) => ({
  ...
  ensureLlamaServerInstalled: installMocks.ensureInstalled,
  resolveManagedLlamaServerPaths: installMocks.serverPaths,
}));
...
describe("managed llama-server", () => {
  ...
  it("writes separate chat and embedding presets without unwired capabilities", async () => {
    const tempRoot = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "llama-preset-")));
    try {
      const presetPath = path.join(tempRoot, "models.ini");
      installMocks.ensureInstalled.mockResolvedValue({
        command: path.join(tempRoot, "llama-server"),
        asset: selectLlamaServerAsset(),
      });
      installMocks.serverPaths.mockReturnValue({ presetPath });
