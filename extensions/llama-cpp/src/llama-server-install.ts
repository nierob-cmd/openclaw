...
export async function downloadVerifiedFile(params: {
  ...
  try {
    const response = await fetch(params.url, { redirect: "follow", signal: controller.signal });
    ...
    if (expectedSha256 && actualSha256 !== expectedSha256.toLowerCase()) {
      throw new Error(`download SHA-256 mismatch: expected ${expectedSha256}, got ${actualSha256}`);
    }
